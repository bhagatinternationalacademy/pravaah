...
def book_gate_pass(request):
    """
    Admin fills visitor form → save → generate PDF → email via commonservices.
    Browser also downloads the PDF directly after form submit.
    """
    if request.method == 'POST':
        form = VisitorForm(request.POST)
        ...
        if form.is_valid():
            ...
            # ── Step 2: Send email via commonservices ─────────────────────
            if pdf_bytes:
                try:
                    from commonservices.email_service import send_gate_pass_email
                    sent = send_gate_pass_email(visitor, pdf_bytes)
                    if sent:
                        messages.success(
                            request,
                            f'✅ Gate pass created and emailed to {visitor.visitor_email}.'
                        )
                    else:
                        messages.warning(
                            request,
                            '✅ Gate pass created. '
                            'Email not sent — no email address was provided.'
                        )
                except ImportError:
                    messages.success(
                        request,
                        '✅ Gate pass saved. '
                        'Email service (commonservices) is not connected yet.'
                    )
                except Exception as exc:
                    messages.warning(
                        request,
                        f'✅ Gate pass saved but email failed: {exc}'
                    )

            # ── Step 3: Serve PDF as download ─────────────────────────────
            if pdf_bytes:
                from django.http import HttpResponse
                filename = f'GatePass_{visitor.gate_pass_id()}.pdf'
